...
static
void set_true_bit_DrvGtkKeybordState(struct DrvGtkKeybordState* a, guint32 shift)
{
	shift &= 0xFFFF;
...
// a = !a
static
void not_DrvGtkKeybordState(struct DrvGtkKeybordState* a)
{
	gint i = 8;
	while(i-->0){
		a->value[i] = ~(a->value[i]);
...
// a = 0
static
void zero_DrvGtkKeybordState(struct DrvGtkKeybordState* a)
{
	gint i = 8;
	while(i-->0){
		a->value[i] = 0;
...
static
struct DrvGtkKeybordState* new_key_transform_table_ja(void)
{
...
static
struct DrvGtkKeybordState* new_key_transform_table_en(void)
{
	struct DrvGtkKeybordState* a = g_malloc0(sizeof(*a) * 0xFFFF);
...
static
struct DrvGtkKeybordState* new_key_transform_table_null(void)
{
	struct DrvGtkKeybordState* a = g_malloc0(sizeof(*a) * 0xFFFF);
