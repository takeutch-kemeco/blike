...
inline struct BL3D_VECTOR* bl3d_mul_vector(
	struct BL3D_VECTOR* dst,
	struct BL3D_VECTOR* src
)
{
	BL3D_MUL_VECTOR(dst, src);
...
/// スカラー倍したい場合も、ベクトルにして乗算する必要がある
/// そのベクトル化したスカラー量を使いまわせるならば、高速化を期待できる
inline struct BL3D_VECTOR* bl3d_mul2_vector(
	struct BL3D_VECTOR* dst,
	struct BL3D_VECTOR* src0,
	struct BL3D_VECTOR* src1
)
{
	BL3D_MUL2_VECTOR(dst, src0, src1);
...
/// esrc - ssrc = dst
/// esrc が終点、 ssrc が始点 
inline struct BL3D_VECTOR* bl3d_diff_vector(
	struct BL3D_VECTOR* dst,
	struct BL3D_VECTOR* esrc,
	struct BL3D_VECTOR* ssrc
)
{
	BL3D_DIFF_VECTOR(dst, esrc, ssrc);
